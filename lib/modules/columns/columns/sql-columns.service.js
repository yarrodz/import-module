...
class SQLColumnsService {
    ...
    async checkIdColumnUniqueness(impt) {
        ...
        try {
            ...
            switch (target) {
                case database_import_target_enum_1.DatabaseImportTarget.TABLE:
                    const tableQuery = (0, sql_query_builder_1.createCheckTableColumnUniquenessQuery)(source, idColumn, table);
                    isUnique = await sqlConnection.queryResult(tableQuery);
                    break;
                case database_import_target_enum_1.DatabaseImportTarget.CUSTOM_SELECT:
                    const customSelectQuery = (0, sql_query_builder_1.createCheckSelectColumnUniquenessQuery)(source, idColumn, customSelect);
