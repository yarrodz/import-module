        ...
        try {
            ...
            switch (target) {
                case sql_import_target_enum_1.SqlImportTarget.TABLE: {
                    try {
                        columns = await this.selectColumnsFromSchema(sqlConnector, table, source);
                ...
                case sql_import_target_enum_1.SqlImportTarget.SELECT: {
                    const query = (0, sql_query_builder_1.paginateQuery)(source, select, idColumn, 0, 1);
                    columns = await this.selectColumnsFromDataset(sqlConnector, query);
                    break;
                }
                default: {
    ...
    async checkIdColumnUniqueness(impt) {
        ...
        try {
            ...
            switch (target) {
                ...
                case sql_import_target_enum_1.SqlImportTarget.SELECT: {
                    const query = (0, sql_query_builder_1.createCheckSelectColumnUniquenessQuery)(source, idColumn, select);
                    isUnique = await sqlConnector.queryResult(query);
                    break;
                }
                default: {
